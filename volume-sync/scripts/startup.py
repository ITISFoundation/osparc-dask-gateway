...
).upper()  # Sync type for distributed sync
if not SYNC_TYPE in SYNC_TYPES:
    logging.info(
        "%s is no valid sync type. Set to default %s" % (SYNC_TYPE, SYNC_TYPE[0])
    )
    SYNC_TYPE = SYNC_TYPES[0]
WAIT_BEFORE_SYNC = int(
    os.getenv("WAIT_BEFORE_SYNC", "10")
)  # Wait before servers are ready
